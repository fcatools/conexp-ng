	...
	protected Void doInBackground() throws Exception {
			...
			if (!context.getDontConsideredAttr().contains(s)) {
				...
				for (FullObject<String, String> f : context.getObjects()) {
					if (f.getDescription().getAttributes().contains(s)
							&& (!context.getDontConsideredObj().contains(f))) {
		...
		for (String s : extentPerAttr.keySet()) {
			for (String t : extentPerAttr.keySet()) {
				if (!s.equals(t)) {
					Set<String> result = context.intersection(
			...
			setProgress((int) (((float) progress++ / extentPerAttr.keySet()
					.size()) * 99));
		}
				...
				for (FullObject<String, String> i : context.getObjects()) {
					if (!context.getDontConsideredObj().contains(i)) {
						if (e.contains(i.getIdentifier().toString())) {
							TreeSet<String> prev = context.sort(i
									.getDescription().getAttributes());
							if (count > 0) {
	...
	protected void done() {
		...
		if (!isCancelled()) {
			...
			if (lattice) {
				...
				setProgress(99);
				setProgressBarMessage(StatusMessage.CALCULATINGLATTICE
						.toString());
				state.lattice = view.getAlg().computeLatticeGraph(
						state.concepts, view.getBounds());
			} else {
