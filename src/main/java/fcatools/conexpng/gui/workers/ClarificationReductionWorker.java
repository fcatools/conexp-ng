...
public class ClarificationReductionWorker extends AbstractWorker {
	 ...
	 */
	public ClarificationReductionWorker(Conf state, ContextMatrix matrix,
			Long progressBarId, boolean type, boolean reduction,
			boolean contextReduction) {
		super(progressBarId);
		this.state = state;
	...
	private void clarify() {
		...
		state.startCalculation(type ? StatusMessage.CLARIFYINGOBJECTS
				: contextReduction ? StatusMessage.CLARIFYING
						: StatusMessage.CLARIFYINGATTRIBUTES);
		setProgressBarMessage(type ? StatusMessage.CLARIFYINGOBJECTS.toString()
				: contextReduction ? StatusMessage.CLARIFYING.toString()
						: StatusMessage.CLARIFYINGATTRIBUTES.toString());
		int progress = 0;
		setProgress(0);
		// start algorithm
		...
		// end calculation
		state.endCalculation(type ? StatusMessage.CLARIFYINGOBJECTS
				: contextReduction ? StatusMessage.CLARIFYING
						: StatusMessage.CLARIFYINGATTRIBUTES);
	}
	...
	public void reduce() {
		// start calculation and set progress bar
		state.startCalculation(type ? StatusMessage.REDUCINGOBJECTS
				: contextReduction ? StatusMessage.REDUCING
						: StatusMessage.REDUCINGATTRIBUTES);
		setProgressBarMessage(type ? StatusMessage.REDUCINGOBJECTS.toString()
				: contextReduction ? StatusMessage.REDUCING.toString()
						: StatusMessage.REDUCINGATTRIBUTES.toString());
		int progress = 0;
		setProgress(0);
		...
		for (int i = 0; i < context.getObjectCount(); i++) {
			...
			ICombinatoricsVector<FullObject<String, String>> otherObjects = Factory
					.createVector(otherObjects0);
			Generator<FullObject<String, String>> gen = Factory
					.createSubSetGenerator(otherObjects);
			for (ICombinatoricsVector<FullObject<String, String>> subSet : gen) {
				if (subSet.getSize() < 2)
					continue;
				IndexedSet<String> intersection = new ListSet<>();
				for (String attribute : context.getAttributesForObject(subSet
						.getValue(0).getIdentifier())) {
					intersection.add(attribute);
				}
				for (int j = 1; j < subSet.getSize(); j++) {
					intersection.retainAll(context
							.getAttributesForObject(subSet.getValue(j)
									.getIdentifier()));
				}
				if (intersection.size() == 0)
					continue;
				if (context.getAttributesForObject(o.getIdentifier()).equals(
						intersection)) {
					toBeRemoved.add(o);
					break;
				}
		...
		// end calculation
		state.endCalculation(type ? StatusMessage.REDUCINGOBJECTS
				: contextReduction ? StatusMessage.REDUCING
						: StatusMessage.REDUCINGATTRIBUTES);
	}
	...
	protected void done() {
		...
		if (!isCancelled()) {
			((ContextMatrixModel) matrix.getModel())
