...
public class ContextEditor extends View {
	...
	public ContextEditor(final Conf state) {
		...
		scrollPane.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				invokeAction(ContextEditor.this,
						matrix.getActionMap().get("selectNone"));
	...
	private void createKeyActions() {
		...
		im.put(getKeyStroke(KeyEvent.VK_G, KeyEvent.SHIFT_MASK),
				"gotoLastObject");
		...
		im.put(getKeyStroke(KeyEvent.VK_DOWN, KeyEvent.SHIFT_MASK),
				"selectDown");
		im.put(getKeyStroke(KeyEvent.VK_LEFT, KeyEvent.SHIFT_MASK),
				"selectLeft");
		im.put(getKeyStroke(KeyEvent.VK_RIGHT, KeyEvent.SHIFT_MASK),
				"selectRight");
		...
		im.put(getKeyStroke(KeyEvent.VK_D, KeyEvent.SHIFT_MASK),
				"removeAttribute");
	...
	private void createButtonActions() {
		...
		group = new WebButtonGroup(WebButtonGroup.VERTICAL, true,
				createButton("addObject", "Add Object",
						"icons/context editor/add_object.png",
						am.get("addObjectAtEnd")), createButton(
						"clarifyObjects", "Clarify Objects",
						"icons/context editor/clarify_objects.png",
						am.get("clarifyObjects")), createButton(
						"reduceObjects", "Reduce Objects",
						"icons/context editor/reduce_objects.png",
						am.get("reduceObjects")));
		...
		group = new WebButtonGroup(WebButtonGroup.VERTICAL, true, createButton(
				...
				am.get("addAttributeAtEnd")), createButton("clarifyAttributes",
				"Clarify Attributes",
				"icons/context editor/clarify_attributes.png",
				am.get("clarifyAttributes")), createButton("reduceAttributes",
				"Reduce Attributes",
				"icons/context editor/reduce_attributes.png",
				am.get("reduceAttributes")));
		group.setButtonsDrawFocus(false);
		toolbar.add(group);
		toolbar.addSeparator();
		group = new WebButtonGroup(WebButtonGroup.VERTICAL, true, createButton(
				"reduceContext", "Reduce Context",
				"icons/context editor/reduce_context.png", am.get("reduce")),
				createButton("transposeContext", "Transpose Context",
		...
		group = new WebButtonGroup(WebButtonGroup.VERTICAL, false,
				createToggleButton("compactMatrix", "Compact Matrix",
						"icons/context editor/compact.png",
						(ItemListener) am.get("compact")), createToggleButton(
						"showArrowRelations", "Show Arrow Relations",
	...
	private void createContextMenuActions() {
		...
		addMenuItem(attributeCellPopupMenu, "Add left",
				am.get("addAttributeLeft"));
		addMenuItem(attributeCellPopupMenu, "Add right",
	...
	class ExpandSelectionAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			if (matrix.isRenaming)
				return;
			matrix.saveSelection();
			int lastActiveRowIndexAfter = clamp(lastActiveRowIndex + vertical,
					1, state.context.getObjectCount());
			int lastActiveColumnIndexAfter = clamp(lastActiveColumnIndex
					+ horizontal, 1, state.context.getAttributeCount());
			...
			boolean isNewRowIndexInsideOldSelection = lastActiveRowIndexAfter >= matrix
					.getLastSelectedRowsStartIndex()
					&& lastActiveRowIndexAfter <= matrix
							.getLastSelectedRowsEndIndex();
			boolean isNewColumnIndexInsideOldSelection = lastActiveColumnIndexAfter >= matrix
					.getLastSelectedColumnsStartIndex()
					&& lastActiveColumnIndexAfter <= matrix
							.getLastSelectedColumnsEndIndex();
			if (wasRowChange) {
				if (isNewRowIndexInsideOldSelection) {
					matrix.removeRowSelectionInterval(lastActiveRowIndex,
							lastActiveRowIndex);
				} else {
					matrix.addRowSelectionInterval(lastActiveRowIndexAfter,
							lastActiveRowIndexAfter);
				}
				lastActiveRowIndex = lastActiveRowIndexAfter;
			}
			if (wasColumnChange) {
				if (isNewColumnIndexInsideOldSelection) {
					matrix.removeColumnSelectionInterval(lastActiveColumnIndex,
							lastActiveColumnIndex);
				} else {
					matrix.addColumnSelectionInterval(
							lastActiveColumnIndexAfter,
							lastActiveColumnIndexAfter);
				}
	...
	class MoveObjectOrAttributeAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			boolean wasColumnChange = (lastActiveColumnIndexAfter - lastActiveColumnIndex) != 0;
			if (wasRowChange) {
				matrixModel.reorderRows(lastActiveRowIndex,
						lastActiveRowIndexAfter);
			...
			if (wasColumnChange) {
				matrixModel.reorderColumns(lastActiveColumnIndex,
						lastActiveColumnIndexAfter);
				matrixModel.fireTableDataChanged();
				lastActiveColumnIndex = lastActiveColumnIndexAfter;
				matrix.selectCell(lastActiveRowIndex, lastActiveColumnIndex);
	...
	class ClarifyObjectsAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			ClarificationReductionWorker crw = new ClarificationReductionWorker(
					state, matrix, progressBarId, true, false, false);
			crw.addPropertyChangeListener(new StatusBarPropertyChangeListener(
	...
	class ClarifyAttributesAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			Long progressBarId = state.getStatusBar().startCalculation();
			ClarificationReductionWorker crw = new ClarificationReductionWorker(
					state, matrix, progressBarId, false, false, false);
			crw.addPropertyChangeListener(new StatusBarPropertyChangeListener(
					progressBarId, state.getStatusBar()));
			crw.execute();
		}
	...
	class ReduceObjectsAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			Long progressBarId = state.getStatusBar().startCalculation();
			ClarificationReductionWorker crw = new ClarificationReductionWorker(
					state, matrix, progressBarId, true, true, false);
			crw.addPropertyChangeListener(new StatusBarPropertyChangeListener(
					progressBarId, state.getStatusBar()));
			crw.execute();
		}
	...
	class ReduceAttributesAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			Long progressBarId = state.getStatusBar().startCalculation();
			ClarificationReductionWorker crw = new ClarificationReductionWorker(
					state, matrix, progressBarId, false, true, false);
			crw.addPropertyChangeListener(new StatusBarPropertyChangeListener(
					progressBarId, state.getStatusBar()));
			crw.execute();
		}
	...
	class ReduceAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			Long progressBarId = state.getStatusBar().startCalculation();
			ClarificationReductionWorker crw = new ClarificationReductionWorker(
					state, matrix, progressBarId, false, false, true);
			crw.addPropertyChangeListener(new StatusBarPropertyChangeListener(
					progressBarId, state.getStatusBar()));
			crw.execute();
		}
	...
	class RenameActiveObjectAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			final String name = state.context.getObjectAtIndex(
	...
	class RenameActiveAttributeAction extends AbstractAction {
		public void actionPerformed(ActionEvent e) {
			...
			// This code is the way it is to work around the inability of
			// flushing the keyboard buffer in Swing
			final String name = state.context
					.getAttributeAtIndex(lastActiveColumnIndex - 1);
			final JTextField t = matrix
					.renameColumnHeader(lastActiveColumnIndex);
			Timer timer = new Timer(0, new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					t.setText(name);
					t.selectAll();
