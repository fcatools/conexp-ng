...
public class DependencyView extends View {
	...
	public DependencyView(final Conf state) {
		...
		scroll2.getViewport().addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				state.guiConf.assoscrollpos = scroll2.getVerticalScrollBar()
						.getValue();
			}
		});
		...
		scroll1.getViewport().addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent e) {
				state.guiConf.implscrollpos = scroll1.getVerticalScrollBar()
						.getValue();
			}
		});
		final WebSplitPane splitPane = new WebSplitPane(
				WebSplitPane.VERTICAL_SPLIT, scroll1, scroll2);
		...
		super.init();
		this.splitPane
				.setDividerLocation(state.guiConf.dependenciessettingssplitpos);
		this.splitPane.addDividerListener(new ComponentListener() {
			...
			@Override
			public void componentMoved(ComponentEvent arg0) {
				state.guiConf.dependenciessettingssplitpos = DependencyView.this.splitPane
						.getDividerLocation();
			}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (evt instanceof ContextChangeEvent) {
			...
			case NEWCONTEXT: {
				...
				((DependencySettings) settings.getComponent(0))
						.setGuiConf(state.guiConf);
				break;
			}
			case CONTEXTCHANGED: {
				state.associations = new TreeSet<>();
	...
	private void updateAssociations() {
		...
		associationWorker = new AssociationWorker(this, state,
				state.guiConf.support, 0, progressBarId);
		associationWorker
				.addPropertyChangeListener(new StatusBarPropertyChangeListener(
		...
		if (!state.associations.isEmpty())
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					if (!state.guiConf.lexsorting) {
						Collections.sort(t, new Comparator<AssociationRule>() {
							...
							public int compare(AssociationRule o1,
									AssociationRule o2) {
								int support1 = state.context.supportCount(o1
										.getPremise());
								int support2 = state.context.supportCount(o2
										.getPremise());

								if (support1 == support2)
									return (int) (o2.getConfidence() - o1
											.getConfidence());
								else
					...
					try {
						assopane.getDocument().insertString(
								assopane.getDocument().getLength(),
						...
						for (AssociationRule asso : t) {
							...
							if (asso.getConfidence() >= state.guiConf.confidence) {
								...
								buf.append(i);
								buf.append("< "
										+ state.context.supportCount(asso
												.getPremise()) + " > ");
								buf.append(asso.getPremise()
										+ " =["
										+ new BigDecimal(asso.getConfidence())
												.setScale(
														3,
														BigDecimal.ROUND_HALF_UP)
												.doubleValue() + "]=> ");
								Set<String> temp = new HashSet<>(asso
										.getConsequent());
								...
								assopane.getDocument().insertString(
										assopane.getDocument().getLength(),
										buf.toString(),
										dependencyStyle(asso.getSupport(),
												asso.getConfidence()));
							}
						}
						((WebScrollPane) ((WebSplitPane) view)
								.getBottomComponent()).getVerticalScrollBar()
								.setValue(assoscrollpos);
						((DependencySettings) settings.getComponent(0)).update(
								i, state.associations.size());
		...
		if (!state.implications.isEmpty()) {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					try {
						implpane.getDocument()
								.insertString(
										implpane.getDocument().getLength(),
						...
						((WebScrollPane) ((WebSplitPane) view)
								.getTopComponent()).getVerticalScrollBar()
								.setValue(implscrollpos);
					} catch (BadLocationException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
